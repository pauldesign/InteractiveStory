...
//  InteractiveStory
//
//  Created by Paul Maxeiner on 8/5/16.
...
extension Story {
    ...
    var soundEffectURL: NSURL {
        let fileName: String
        switch self {
        case .Droid, .Home: fileName = "Sounds/HappyEnding"
        case .Monster: fileName = "Sounds/Ominous"
        default: fileName = "Sounds/PageTurn"
        }
...
struct Adventure {
    static func story(name: String) -> Page {
        ...
        let touchdown = returnTrip.addChoice("Stop and invesitgate", story: .TouchDown)
        ...
        cave.addChoice("Continue towards the light", story: .Droid(name))
